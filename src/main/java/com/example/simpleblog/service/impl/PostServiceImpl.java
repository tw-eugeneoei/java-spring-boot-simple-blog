...
import com.example.simpleblog.aspect.*;
...
public class PostServiceImpl implements PostService {
  ...
  public PostResponse getPosts(int pageNo, int pageSize, String sortBy, String sortDir) {
    ...
    // Pageable pageable = PageRequest.of(offsetPageNo, pageSize); // create Pageable instance with
    // page number and page size
    // Pageable pageable = PageRequest.of(offsetPageNo, pageSize, Sort.by(sortBy)); // create
    // Pageable instance with page number, page size, and sort value
    ...
    // List<PostDto> results = listOfPosts.stream().map(post ->
    // mapToDTO(post)).collect(Collectors.toList());
    List<PostDto> results =
        listOfPosts.stream().map(this::mapToDTO).collect(Collectors.toList()); // same as above
  ...
  @EgAfterThrowing()
  @EgAfter(value = "Value for EgAfter AOP annotation")
  @EgAround(value = "Tony Stark")
